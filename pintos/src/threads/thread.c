...
#define TIME_SLICE 4            /* # of timer ticks to give each thread. */
				   ...
				   thread_func *function, void *aux) 
	{
	  ...
	  // XXX : Set Parent. (Initialize)
	  t->parent = thread_current();
	  //printf("Add New Parent %s to %s\n", t->parent->name, t->name);
	  // XXX
	...
	thread_block (void) 
	{
	  ...
	  //printf("[BLOCK] %s ", thread_current()->name);
	...
	// find thread in ALL_LIST by tid and return at aux.
	...
	bool FD_List_Less_Func (const struct list_elem *a,
                            ...
                            void *aux UNUSED){
		struct fd_list *fd_a = list_entry(a, struct fd_list, elem);
		struct fd_list *fd_b = list_entry(b, struct fd_list, elem);
		if(fd_a->fd < fd_b->fd)
			return true;
		else
			return false;
	...
	init_thread (struct thread *t, const char *name, int priority)
	{
	  ...
	  t->priority = priority;
	  // XXX : Initialize 'Child List' & 'Semaphore'
	  list_init(&(t->childs));
#ifdef USERPROG
	  sema_init(&(t->sema), 0);
#endif
